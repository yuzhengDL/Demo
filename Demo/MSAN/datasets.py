...
def load_dataset(path, name, load_train=True):
    """
    ...
    if load_train:
        with open(path+name+'_train_caps.txt', 'rb') as f:
            for line in f:
    ...
    with open(path+name+'_test_caps.txt', 'rb') as f:
        for line in f:
            test_caps.append(line.strip())
    # Image features
    if load_train:
        train_ims_local = np.load(path+name+'_train_ims_local.npy')
        train_ims_global = np.load(path+name+'_train_ims_global.npy')
        #train_ims_global = preprocessing.scale(train_ims_global)
        train_ims_NIC = np.load(path+name+'_train_ims_NIC.npy')
    ...
    test_ims_local = np.load(path+name+'_test_ims_local.npy')
    test_ims_global = np.load(path+name+'_test_ims_global.npy')
    #test_ims_global = preprocessing.scale(test_ims_global)
    test_ims_NIC = np.load(path+name+'_test_ims_NIC.npy')
